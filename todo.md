...
6. 电子围栏（无法查找到Geometry的顶点属性： **解决**）
7. 多边形标注特效（无法查找到Geometry的顶点属性： **解决**）
8. 模型特效：添加模型边缘发光按钮点击无效
9. 轨迹线特效：无法添加轨迹线特效
10. 点云线特效：
11. 区域围墙特效：（特效组件问题）
...
它是一个功能强大且易于使用的动画库，适用于需要在 数字孪生应用中实现复杂动画效果的开发者。
...
tileService中的queryTilesByNum将需要加载的瓦片记录至this.currentView、this.secondViews后，后续没有针对该数组的处理措施

count计数纠正

纠正tileService中主动强制加载瓦片接口，解决queryTilesByNum接口触发不稳定的问题，纠正瓦片地图建筑加载数量统计逻辑

重新校正城市地图中加载的瓦片建筑统计方法。

对广哈新调整后的地图与室外地面模型进行统一测试，排查所有网格是否缺失floor1属性，将异常情况通过标注体现在场景中。

回溯raycastByCoord接口的射线拾取逻辑，调查模型子构件无法被选中且该接口无法返回正确值的原因

对模型新增location属性

碰撞检测中 showlevel 、forceVisible、碰撞检测之间的逻辑

原始

## 11.14

排查图文标注创建后立即修改后无法全部生效的问题，并根据实际应用情况的不同给出详细的解决方案

## 11.15

细化碰撞检测的两两比较机制，完善碰撞组与待定组的比较与推入逻辑，确认之前偶尔出现的标签重合原因，重构MapBoxCollision类，避免出现性能问题。

针对用户提出的根据外部容器宽高自适应调整场景缩放层级的问题，提出三版解决方案并实践得出具体效果（排除镜头移动与宽高比例适应方案）

复现图文标注创建后立即修改后无法全部生效的问题，并重新完善异步流程实现目标效果
...
1. 快速创建行政区域并实时更新整体样式的工具。（未完成）【共两个工具：编辑器工具、加载工具】
2. 相机分镜动画工具（进度80%）【差相机操控交互难度大问题】
3. 画面分屏现已解决相同场景的展示（含交互动作），未解决不同场景和画面形状定制问题（目前暂不支持不同场景，目前是两个子视口共用一个渲染器所以仅能通过内部[setViewport](https://threejs.org/docs/index.html#api/zh/renderers/WebGLRenderer.setViewport) 、.[setScissor](https://threejs.org/docs/index.html#api/zh/renderers/WebGLRenderer.setScissor) 接口设置子视口，无法定制画面形状，开启模板测试方案）
4. 第一人称的操作交互修正。

## 11.19

第三人称模式：优化代码整体结构与车辆运动逻辑，车辆模式下动画播放逻辑重构，解决车辆停止运动但动画播放不会停止的问题，解决车辆运动惯性控制不自然和模型异常抖动的问题

新增重力加速度机制、削减控制器镜头阻尼

人模式：射线 : 正常更新高度  车模式更新高度和前后倾角

## 11.20

第三人称模式：调整射线向下探测获取高度的拾取逻辑。保持运动过程中控制目标与相机之间的相对一致。

增加运动过程中的高度控制，解决地面高度无法拾取的问题，解决添加模型高度偏移量之后控制目标抖动的问题

增加车辆模式下俯仰角控制机制。调整人员与车辆的相关控制参数

## 11.21

完成第三人称行人与车辆模式下的运动参数的默认设置，完成第三人称主体功能。

新建第一人称控制器，完成第一人称的初始控制逻辑设计，完成第一人称初始化、事件绑定、移动与相机姿态变换的逻辑基础功能，解决相机的六个方向向量获取异常的问题。

保留自由视角飞行控制逻辑。

## 11.22

完成第一人称控制器的主体功能，完成第一人称下的鼠标拖动控制逻辑。

删除SDK中原有的第一人称相关的开关及其他加载更新逻辑，重新接入新的第一人称控制，防止进入场景后控制器无法正常生效的问题

优化相机分镜动画工具下相机up向量变化，防止出现镜头角度过零点时大幅度翻转的现象

## 11.25

目前问题：需要实时更新target对象来适应原有第一人称控制器在其他场景中的逻辑。

射线检测列表选取 `rayPointer.watchingList`(性能较差，会出现卡顿)，`mapPicker.locationModelMesh`（列表更新时机不明确）

float_point 漂浮点特效 修改无反应

用户反映无法使图文标注批量更新标注发光，编写相应测试示例，

增加第一人称控制器目标视点，适应原有第一人称的逻辑，防止出现兼容问题
...
```javascript
   ...
   * @type {boolean}
   */
  set firstPersonMode(value) {
    if (value) {
      if (!this.firstPersonControls) {
        this.firstPersonControls = new esmap.FirstPersonControls(
          this.currentCamera,
  ...
  get firstPersonMode() {
    return !!(this.firstPersonControls && this.firstPersonControls.enabled);
  }

```

增加第一人称控制模式、恢复原有第一人称控制逻辑

解决图文标注高度无法统一修改更新的问题

基于原有项目创建全国行政区域边界快速编辑工具

修复多边形标注几何实体无法创建的问题

## 11.27

解决内蒙古与河北省行政区域绘制出错的问题

修改阴影地板边界边距计算逻辑，解决阴影底版与行政区域不贴合的问题，完成全国行政区划绘制与阴影底版

删减该项目原有冗余代码与无关逻辑

问题：多边形标注的性能问题(**340个多边形标注 fps降至20以下**)，

    底版阴影纹理闪烁问题（高度分离）

## 11.28

- [X] 数据抽稀（对于每个点位间距控制 点位个数）优化页面性能
- [X] 阴影底版的颜色配置
- [X] 多边形线条样式配置
- [X] 纠正multiPolygon属性的数据处理流程
- [X] 省会用图文标注

气泡、定位、三维模型标注不用设置、线标注配置集成到多边形标注中

特效标注只用关注 区域围墙、浮动点标注

## 11.29

针对各行政区域数据点集进行抽稀算法处理，优化场景渲染性能

增加阴影底版的相关样式配置、增加多边形标注的边界线条样式配置，并进行相关测试

针对原始数据中内蒙古自治区的特殊数据纠正现有的数据处理流程

省级行政区名称更改为图文标注表示、删减其他不必要的标注样式配置。

区域围墙部分样式设置无效

全国-省-市-区县-街道  用单个例子给出垂直的细分流程

建立省级底版阴影失败？

行政区域缩放退出

修复stylecconfig类中的线段类型和可见性问题

新增广东各省市及深圳各区县的行政区划，完善相应的处理流程

完善底版阴影快速生成流程，处理顶点集合中非必要的脏数据

完善标注样式插件中标注样式更新流程，解决标注样式错乱及显隐问题

对于ios设备与安卓设备上的文字标注及dom大小显示异常进行了相关测试 并给出了相应的情况报告
...
解决在修改边界线段宽度的同时线条样式也发生变化的问题

解决第二次修改阴影后阴影消失的问题，解决阴影地板重建时颜色错乱的问题

修复区域围墙颜色修改出错问题

- [X] 底版阴影重建参数偶尔会报空
- [X] 修改样式的时候添加loading
- [X] 行政区域层级 的marker 厚度（样式插件控制）
- [X] 层级控制marker显隐
- [X] 添加图文标注的样式控制机制
...
修改标注样式的时候添加loading效果

排查并解决LGJF-001场景中开启模型懒加载后仅加载某一楼层的问题

排查并解决LGJF-001场景中关闭楼层加载动画后第二次主动触发楼层加载无反应的问题（openBuilding接口）

## 12.4

利用地图层级缩放控制省市区各标注显隐

添加图文标注的样式控制机制，完成行政区划及标注样式编辑工具

解决区域围墙标注、浮动点标注参数修改后无法正常更新的问题

调整工具内部marker队列更新机制，避免重复更新

## 12.5

排查LGJF-001场景中cameraFlyTo视角飞入后场景加载卡顿的问题，检查场景中模型加载队列与顶点、纹理加载情况。

排查checkPanLimit接口执行后镜头徘徊飘动的问题，重构边界超出的判定方式与超出边界后的调整方式

## 12.6

画面分屏现已解决相同场景的展示（含交互动作），未解决不同场景和画面形状定制问题（目前暂不支持不同场景，目前是两个子视口共用一个渲染器所以仅能通过内部[setViewport](https://threejs.org/docs/index.html#api/zh/renderers/WebGLRenderer.setViewport) 、.[setScissor](https://threejs.org/docs/index.html#api/zh/renderers/WebGLRenderer.setScissor) 接口设置子视口，无法定制画面形状，开启模板测试方案）

画面分屏接口：

创建圆形副屏及其外围dom包围盒相关属性适配，

针对圆形副屏完成鼠标事件适配、射线拾取适配，

测试分屏中正交、投影相机、控制器再在体场景中的运行

测试导出读取标注演示配置文件，优化初次重建部分标注时的逻辑

## 12.7

对于部分标注重建时的异步流程及相关逻辑进行重构，完成READ模式下读取配置文件并重建marker的功能

测试第一人称在分屏情况下的应用（目前未发现问题）
...
增加行人模式下，自动纠正模型前进方向与相机正前方的偏角

排查并修复键盘操控时相机与模型运动不同步的问题

第一人称控制器：

第一人称控制器增加固定高度模式

增加右键点击模式自动拾取点位，自动计算方向并执行动画移动至目标点位

## 12.11

问题1：向指定位置移动 与 自动纠正模型姿态相冲突

## 12.12

解决向指定位置移动功能 与 自动纠正模型姿态功能相冲突的问题

解决tween动画的onUpdate回调中数值的更新问题

解决模型转向异常的问题

完善鼠标操作模式下车辆运动动作的执行逻辑
...
鼠标操作模式下 鼠标点击过快的话会出现转向不及时的情况

增加鼠标控制模式下的行人和车辆的模型姿态矫正，解决鼠标点击过快时出现的模型朝向与运动方向不一致的问题

调整运动过程中模型运动动画的权重解决重新确定终点后动画重播的问题
...
第三人称控制器增加鼠标控制模式下的目标点位选取指示器及其增减逻辑

解除相机的缩放与旋转姿态锁定、增加相机与模型之间的距离锁定

测试第一人称在城市底图与室内地图的功能运行情况

相机分镜工具接入第一与第三人称控制器

## 12.17

楼层内模型实际高度与实际位置不符合（模型加载至某个模型后可能会出现高度偏移的问题）

配置LOD之后模型加载不完全

利用各个模型分级加载的方式 使各个模型逐个加载杜绝一次性全部加载带来的卡顿

测试LGJF-001项目各个模型加载的性能消耗
...
优化视角飞入之后城市地图内的模型无法完全加载的问题、解决进入场景后mergeQueue无法触发加载的问题
...
1._`this.mapModelsCount<_this.modelStatic.roomModelsCount`条件判断不成功

2.该部分模型未能成功进入modelStore队列，导致后续在startLoadModelsByFLoor函数中无法加载

raycastByCoord 判断条件增加 targetModel

增加 对模型targetModel判断逻辑避免raycastByCoord无法选取地面高度的问题

分析江苏帝一工厂室内模型加载流程,新增接口:将部分未能成功加载的模型重新回流至正常加载流程

## 12.23

第三人称控制器：根据现有模型情况，获取人物模型前方向，调整行人模式、车辆模式、鼠标控制模式、键盘控制模式、转向自动矫正模式下的模型姿态，并进行相关测试

创建模型加载队列，添加队列加载的管理与控制逻辑，尝试在模型加载流程中补充另外的加载流程、
...
第一人称控制器：调整控制器更新方式、启动退出机制、事件绑定机制，解决几种控制器之间无法正常切换的问题

解决室内模型大批量加载时出现的部分模型无法加载的问题。

完成分镜工具与第一人称控制器的集成，补充分镜工具使用说明文档

## 12.27

添加自由飞行控制器，解决自由飞行控制器的启动退出问题、指针锁定API的触发问题，调控该控制器的控制参数，使控制起来更自然。

第一人称控制器增加重力加速度机制，调整地面碰撞的射线检测机制，解决速度参数调整不生效的问题

## 12.28

分屏新增鹰眼模式，修正第二视口的设置方式。以适应新项目的交付要求
