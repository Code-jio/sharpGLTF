...
// Read from URL.
let document = await io.read("./model/zhusuji/zhusuji.gltf");
...
await document.transform(
  ...
  weld({ tolerance: 0.0001 }), // 顶点合并 tolerance: 0.0001 顶点合并容差
  ...
  // normals({overwrite: true}), // 重新计算法线 overwrite: true 会覆盖原有法线
  reorder({encoder: MeshoptEncoder, level: 'medium'}), // 重新排序顶点索引 level: 'low', 'medium', 'high'
  partition({meshes: true}), // 分区 meshes: true 会将每个网格分区为多个子网格，以便更好地利用GPU缓存
